...
public interface ReportDetailRepository extends JpaRepository<Report_Details, Integer> {
	@Query(value = "SELECT max(report_id) + 1 FROM Report")
	public Integer max();


	@Query(value = "SELECT s.scope_name,TECH.technology_name,t.task_name,RD.effort,RD.scope_Flag FROM Report REP,Report_Details RD, Technology TECH,  Task t,Scope s "
			+ " WHERE REP.report_id = RD.report_id and t.task_id = RD.task_id and t.techid = TECH.techid and TECH.techid = RD.techid AND s.scope_id = RD.scope_id and REP.cust_id = :custId and REP.report_name = :reportName")
